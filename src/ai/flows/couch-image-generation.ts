  ...
  async (input) => {
    ...
    const {media} = await ai.generate({
      ...
      prompt: [
        ...
        {text: 'You are a photoshop expert. The first image is the background. The second image contains a subject. Your task is to perfectly composite the subject from the second image so it is sitting on the white couch seat in the first image. The subject should be placed in the center of the couch, on the couch seat. It is critical that you DO NOT change the background image (the first image) at all. The people in the background must not be removed or altered.'},
      ],
      config: {
